...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 13 20:18:36 2025
...
def schedule_run(run_nb, flag, country, run_name, file_name, folder_name):
    if not flag:
 ...
 #       run_index.write("\n")
    ...
    if args.dry:
        ...
        run_params = {
            ...
            "environment": "ALL",
            "executable": '/p/projects/ebm/inga/tipESM/forcing_cluster.py',
            "options": " --file_name %s --folder_name %s --run_name %s --country %s"%(file_name, folder_name, run_name, country),
...
with zipfile.ZipFile(zip_path, 'r') as zip_ref:
    ...
    # ✅ Extract subfolders inside 'PHL/'
    folders = sorted({
        name.split('/')[1]  # take the second part after 'PHL/'
        for name in all_names
        if name.startswith('PHL/') and '/' in name and len(name.split('/')) > 1
    })

    print(f"Found folders in PHL/: {folders}")

    # Iterate over the folders and runs
    for folder in folders:
        for r in runs:
            run_name = f"{folder}_{country}_{r}"
            folder_name = f"{folder}"
            file_name = f"{folder}_{country}_{r}.csv"
            schedule_run(enum, single, country, run_name, file_name, folder_name)
            enum += 1
            print(enum)
